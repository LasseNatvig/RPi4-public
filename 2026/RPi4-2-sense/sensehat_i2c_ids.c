// sensehat_i2c_ids.c
// TDT4258 - 2026 - RPi4-2-sense - code example demonstrating i2c programming for sensehat sensors
...
static int i2c_open(const char *dev, uint8_t slave_addr) { // opens the I2C device given by slave_addr
    int fd = open(dev, O_RDWR);
    if (fd < 0) {
        fprintf(stderr, "open(%s) failed: %s\n", dev, strerror(errno));
        return -1;
    }
    if (ioctl(fd, I2C_SLAVE, slave_addr) < 0) { // I2C has many devices attached. Read/write operations will now go to given device 
...
static int i2c_read_reg_byte(int fd, uint8_t reg_addr, uint8_t *out_byte) {  
    if (write(fd, &reg_addr, 1) != 1) {  // write the register address (to be read) to the file descriptor fd
        fprintf(stderr, "write(reg=0x%02X) failed: %s\n", reg_addr, strerror(errno));
        return -1;
    }
    uint8_t val = 0;
    if (read(fd, &val, 1) != 1) { // try to read from the given register
...
static int i2c_write_reg_byte(int fd, uint8_t reg_addr, uint8_t value) {
    uint8_t buf[2] = { reg_addr, value }; // puts register specification and value to write into a buffer
    if (write(fd, buf, 2) != 2) { // tries to write both bytes
        fprintf(stderr, "write(reg=0x%02X) failed: %s\n", reg_addr, strerror(errno));
        return -1;
    }
    return 0;
...
static int i2c_read_reg_word_le(int fd, uint8_t reg, int16_t *out) { // reads a 16-bit little-endian register pair
    uint8_t lo, hi;
    if (i2c_read_reg_byte(fd, reg,     &lo) < 0) return -1; // reads low byte from reg
    if (i2c_read_reg_byte(fd, reg + 1, &hi) < 0) return -1; // reads high byte from reg + 1
    *out = (int16_t)((uint16_t)hi << 8 | lo); // combines the two bytes into a 16bit word using little-endian format
    return 0;
}

static int read_hts221_calibration(int fd, double *T0, double *T1, int16_t *t0_out, int16_t *t1_out) {
    // Reads calibration data from the sensor's factory-programmed registers
    ...
    if (i2c_read_reg_byte(fd, HTS221_T0_DEGC_X8, &t0_x8) < 0 ||  // read lower 8 bits of T0
        ...
        i2c_read_reg_word_le(fd, HTS221_T0_OUT_L, t0_out) < 0 || // the raw sensor output (ADC value) at T0
        i2c_read_reg_word_le(fd, HTS221_T1_OUT_L, t1_out) < 0) {
        return 1;
    }
    *T0 = (((msb & 0x03) << 8) | t0_x8) / 8.0;
    *T1 = (((msb & 0x0C) << 6) | t1_x8) / 8.0;
    printf("Calibration: T0=%.3f C (raw=%d), T1=%.3f C (raw=%d)\n", *T0, *t0_out, *T1, *t1_out);
    return 0;
}

/*** Pressure Sensor Functions (LPS22HB and LPS25H) ***/
static int lps_read_id(int fd) { // read identification register of pressure sensors
    uint8_t id;
...
static int lps_init(int fd, int sensor_id) {
    ...
    if (sensor_id == LPS25H_WHO_AM_I) {
        ...
        ctrl_val = (1 << 7) | (1 << 4) | (1 << 2); // See LPS25H datasheet section 7.6
    } else {
...
static int read_pressure(int fd, double *pressure_hpa) {
    // Check if pressure data is ready (status register 0x27, bit 0 = P_DA)
    uint8_t stat;
    if (i2c_read_reg_byte(fd, 0x27, &stat) != 0) {  // read status register
        printf("DEBUG: Failed to read status\n");
        return -1;
    }
    // printf("DEBUG: pressure status=0x%02X\n", stat);
...
int main(void) {
    ...
    // *****************************************************************************************
    // Read HTS221 calibration data from the sensor's factory-programmed registers
    double T0, T1;
    int16_t t0_out, t1_out;
    if (read_hts221_calibration(fd, &T0, &T1, &t0_out, &t1_out) != 0) {
        close(fd);
        return 1;
    }
    ...
    if (ioctl(fd, I2C_SLAVE, LPS22HB_ADDR) >= 0) {
        lps_sensor_id = lps_read_id(fd);  // find out what kind of pressure sensor is on the sense hat
    ...
    for (int i = 0; i < TOTAL_SAMPLES; i++) {
        ...
        for (int j = 0; j < INNER_SAMPLES; j++) {
            ...
            // Line above calculates temperature by calibration data and linear interpolation
            inner_valid++;
        }
